#!/bin/bash
# Bootstrapper to semi-automatically build avidemux from source on macOS Tahoe (arm64)
# (c) Mean 2009
...
usage()
{
        ...
        echo "  --local-deps=PATH       : Use local Homebrew prefix instead of system one (see setupLocalDeps.sh)"
...
while [ $# != 0 ] ;do
    ...
    case "$config_option" in
        ...
        --version=*)
            ...
            ;;
        --local-deps=*)
            local_deps_path=$(option_value "$config_option")
            ;;
        --without-qt)
            do_qt4=0
            ;;
...
# Wire up local deps prefix when --local-deps is given.
if [ -n "$local_deps_path" ] ; then
    if [ ! -d "$local_deps_path" ] ; then
        echo "Error: local deps path \"$local_deps_path\" does not exist."
        echo "Run setupLocalDeps.sh first."
        exit 1
    fi
    echo "Using local deps at: $local_deps_path"
    export PATH="${local_deps_path}/bin:$PATH"
    export MYQT="${local_deps_path}/opt/qt@6"
    EXTRA_CMAKE_DEFS="-DCMAKE_PREFIX_PATH=${local_deps_path} $EXTRA_CMAKE_DEFS"
    export PKG_CONFIG_PATH="${local_deps_path}/lib/pkgconfig:${local_deps_path}/opt/qt@6/lib/pkgconfig:$PKG_CONFIG_PATH"
fi

# If the path to a custom Qt installation is passed via MYQT variable,
# check for a conflicting one from Homebrew.
if [ -n "$MYQT" ] && [ -f "/opt/homebrew/bin/qmake" ] && [ -z "$local_deps_path" ]; then
    echo -e "\n****************************************************************"
...
# doesn't match the one linked into /opt/homebrew, the build will fail.
qmake_location=$(which qmake)
if [ -z "$local_deps_path" ] ; then
    if [ "$qmake_location" = "/opt/homebrew/bin/qmake" ]; then
        ...
        if [ "${QT_EXT}" != "Qt${linked_qt_version_major}" ]; then
            echo -e "\n*********************** Fatal Error ****************************"
            echo -e "Qt version of the installation linked into /opt/homebrew does not"
...
if [ -n "$MYQT" ] && [ -f "${MYQT}/bin/qmake" ] ; then
    ...
    if [ -z "$local_deps_path" ] && [ "$qmake_location" != "/opt/homebrew/bin/qmake" ]; then
        echo -e "\n****************************************************************"
        echo -e "When using Qt6 from Homebrew, please make sure it is linked into"
        echo -e "/opt/homebrew by executing\n"
